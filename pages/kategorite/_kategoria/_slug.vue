      ...
      <div class="responsive mb-8">
          ...
          <div class="details">
              <div class="shithole">
                  ...
                  <p class="price-prod" v-if="product.details.priceLow">{{product.details.priceLow}} <span class="mini-span">ALL</span> <span class="text-decoration-line-through qs s14 grey--text">{{product.details.price}}</span> <span class="mini-span grey--text">ALL</span></p>
                  <p class="price-prod" v-if="product.details.priceLow == null">{{product.details.price}} <span class="mini-span">ALL</span></p>
                  <div class="rowting">
                      <v-btn class="white--text rounded-md width-70" color="primary" @click="addToCart(product.details.name, product.details.seller, product.details.priceLow ? product.details.priceLow : product.details.price, 1, product.details.desc, product.details.photos[0].src)">Add to Cart</v-btn>
...
export default {
    ...
    async asyncData({params, route, $axios}){
        ...
        var obj = await $axios({
            method: "post",
            url: "http://91.230.254.11/products",
            params: {
                "product_name": spot2
            },
